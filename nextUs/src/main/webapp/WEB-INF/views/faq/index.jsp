							...
							<c:forEach var="vo" items="${map.list }">
								<tr>
									...
									<td style="text-align: left;"><a
										href="view.do?no=${vo.no}">${vo.question }</a></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
						...
						<form method="get" name="searchForm" id="searchForm"
							...
							<span class="srchSelect"> <select id="stype"
								name="searchType" class="dSelect" title="검색분류 선택">
									...
									<option value="title"
										<c:if test="${FAQVO.searchType == 'question'}">selected</c:if>>질문</option>
									<option value="content"
										<c:if test="${FAQVO.searchType == 'answer'}">selected</c:if>>답변</option>
							</select>
							</span> <span class="searchWord"> <input type="text" id="sval"
								name="searchWord" value="${FAQVO.searchWord}" title="검색어 입력">
